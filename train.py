	...
	def start(self):
		...
		for numeps in range(self.par['num_episodes']):
			...
			for maxl in range(self.par['episode_max_length']):
				...
				local_cnt+=1	
				self.par['eps'] = max(0.1,1.0 - float(cnt)/float(self.par['eps_step']))
		...
		if not terminal: 
			if np.random.rand() > self.par['eps']:
                ...
                if self.validactions[4]:
                    ...
                    self.Q_temp[4] = -150
                self.Q_global = max(self.Q_global,np.amax(Q_temp))
                return action = self.Q_temp.index(max(self.Q_temp))
		    else:
			    return self.validactions[np.random.randint(0,len(self.validactions))]
